...
runners.forEach((name) => {
  return fullNames.push(`Full Name: ${name.first_name} ${name.last_name}`)
});
...
let ticketPriceTotal = runners.reduce((donations, total) => {
  return donations += total.donation;
}, 0)
...
// Problem 1
let bigBoySizes = runners.filter((threexl) => {
  return threexl.shirt_size === '3XL'
});
console.log(bigBoySizes);

// Problem 2
let contactPeople = runners.map((email) => {
...
runners.forEach((company) => {
  return companyNames.push(`Company Name: ${company.company_name}`)
});
